...
export default class ConsoleView {
  ...
  prompt() {
    return atom.config.get('tidalcycles.console.prompt')
  ...
  logText(text, error) {
    ...
    if (atom.config.get('tidalcycles.console.onlyLogLastMessage')) {
      this.log.innerHTML = "";
    }
    pre.innerHTML = text;
    this.log.appendChild(pre);

    if (!error && atom.config.get('tidalcycles.console.onlyShowLogWhenErrors')) {
