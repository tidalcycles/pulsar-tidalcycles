...
export default class ConsoleView {
  ...
  logMutes(mutes, lastChanged) {
    let entries = Object.entries(mutes)
    let names = entries.map(entry => {
      if (entry[0] === lastChanged) {
        return ` <strong>${entry[0]}</strong> |`
      } else {
        return ` ${entry[0]} |`
      }
    })

    let statuses = entries.map(entry => {
      if (entry[0] === lastChanged) {
        return ` <strong>${entry[1] ? 'M' : 'U'}</strong> |`
      } else {
        return ` ${entry[1] ? 'M' : 'U'} |`
      }
    })

    let firstRow  = 'conn   |' + names.join('')
    let secondRow = 'status |' + statuses.join('')
    this.logStdout(firstRow + '\n' + secondRow)
  }
