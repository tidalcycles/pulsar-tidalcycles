...
export default class REPL {
    ...
    initTidal() {
        ...
        for (var i = 0; i < commands.length; i++) {
          if (commands[i].startsWith("tidal")) {
            alert(commands[i]);
            var v = this.addChaosSettingsToTidalStartCommand();
            alert(v);
            this.tidalSendLine(v);
            alert("sent");
          } else {
    ...
    tidalSendLine(command) {
      if (command.includes("chaos") && !command.startsWith("tidal")) {
    ...
    sendChaosLine(command) {
      ...
      var notes = fs.readFileSync(this.getChaosFilesLocation() + '/Bank/birfXVals-' + channel + '.txt').toString();
    ...
    getChaosFilesLocation() {
      return atom.config.get('tidalcycles.chaosFilesLocation');
    }

    getChaosFunctionToApply() {
      return atom.config.get('tidalcycles.chaosFunctionToApply');
    }

    addChaosSettingsToTidalStartCommand() {
      return 'tidal <- startTidal (superdirtTarget {oLatency = 0.1, oAddress = "127.0.0.1", oPort = 57120})'
      + ' (defaultConfig {cFrameTimespan = 1/20, cChaosFilesLocation = '
      + '"'
      + this.getChaosFilesLocation()
      + '"'
      + ', cChaosFunctionToApply = '
      + '"'
      + this.getChaosFunctionToApply()
      + '"'

      + ', cChaosFunctionXLowBound = '
      + atom.config.get('tidalcycles.chaosFunctionXLowBound')

      + ', cChaosFunctionXUpperBound = '
      + atom.config.get('tidalcycles.chaosFunctionXUpperBound')

      + ', cChaosFunctionRLowBound = '
      + atom.config.get('tidalcycles.chaosFunctionRLowBound')

      + ', cChaosFunctionRUpperBound = '
      + atom.config.get('tidalcycles.chaosFunctionRUpperBound')

      + ', cChaosFunctionRIncrement = '
      + atom.config.get('tidalcycles.chaosFunctionRIncrement')
