...
export default class REPL {
      ...
      this.status = {
        ...
        diff() {
          ...
          let diff = this.evaluations.length > 1
            ...
            : lastEval.characters
          return diff > 0 ? '+' + diff : lastEval.characters
        }
      }
    ...
    flushStd() {
      let consolePrompt = atom.config.get('tidalcycles.consolePrompt')
        .replace("%ec", this.status.evaluations.length)
        .replace("%ts", Math.round(new Date() / 1000))
    ...
    eval(evalType, copy) {
      ...
      this.status.evaluations.push({ characters: expression.length })
