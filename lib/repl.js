...
const EVAL_LINE = 'line'
const EVAL_MULTI_LINE = 'multi_line'
const EVAL_WHOLE_EDITOR = 'whole_editor'
...
export default class REPL {
  ...
  constructor(consoleView, ghc, bootTidal) {
    ...
    atom.commands.add('atom-text-editor', {
      'tidalcycles:eval': () => this.eval(EVAL_LINE, false),
      'tidalcycles:eval-multi-line': () => this.eval(EVAL_MULTI_LINE, false),
      'tidalcycles:eval-whole-editor': () => this.eval(EVAL_WHOLE_EDITOR, false),
      'tidalcycles:eval-copy': () => this.eval(EVAL_LINE, true),
      'tidalcycles:eval-multi-line-copy': () => this.eval(EVAL_MULTI_LINE, true),
  ...
  evalWithRepl(expression, range, copy) {
    var self = this;
    if (!expression || !range) return;

    var unflash = self.evalFlash(range);
    if (copy) {
  ...
  currentEvaluations(evalType) {
    ...
    if (expression) {
      ...
      switch (evalType) {
        ...
        case EVAL_MULTI_LINE:
          return this.getMultiLineExpression(editor);
        case EVAL_WHOLE_EDITOR:
