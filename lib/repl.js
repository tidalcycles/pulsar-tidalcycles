...
export default class REPL {
    ...
    flushStd() {
      let customConsolePrompt = atom.config.get('tidalcycles.consolePrompt')

      if (this.stdErr.length) {
        let output = this.stdErr.join('').trim()
        if (customConsolePrompt) {
          output = output.replace(/<interactive>.*error:/g,"")
            .replace(/ \(bound at.*/g,"")

          output = customConsolePrompt + ">" + output
        }
      ...
      if(this.stdOut.length){
        let output = this.stdOut.join('').trim()

        if (customConsolePrompt) {
          output = output.replace(/tidal>.*Prelude>/g,"")
            .replace(/tidal>/g,"")
            .replace(/Prelude>/g,"")
            .replace(/Prelude.*\|/g,"")
            .replace(/GHCi.*help/g,"")
