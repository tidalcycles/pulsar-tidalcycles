...
export default {
    ...
    activate() {
      ...
      const oscIp = atom.config.get('tidalcycles.oscServer.ip');
      const oscPort = atom.config.get('tidalcycles.oscServer.port');
      this.oscServer = new OscServer(this.consoleView, oscIp, oscPort);
      ...
      if (atom.config.get('tidalcycles.oscEval.enable')) {
        this.oscServer.register(atom.config.get('tidalcycles.oscEval.address'), oscEvalSubscriber(this.repl, this.editors))
      }

      this.soundBrowser = new SoundBrowser();
      this.soundBrowser.init(atom.config.get('tidalcycles.soundBrowser.folders'));

      if (this.oscServer.hasListeners()) {
