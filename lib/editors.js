...
export class Editors extends EventEmitter {
  ...
  currentHighlight(highlights) {
    ...
    let expressions = this.getWholeEditorExpressions(editor)
    ...
    Object.keys(highlights)
      .forEach(id => {
        let expression = expressions.find(expression => expression.id === id)
        let translateBy = expression.range.start.row

        highlights[id].ranges
          .map(range => [[range[0][0]+translateBy, range[0][1]],[range[1][0]+translateBy, range[1][1]]])
          .map(range => editor.markBufferRange(range, { invalidate: 'never' }))
          .forEach(marker => editor.decorateMarker(marker, { type: 'text', class: 'highlight'}))
      })
  }
