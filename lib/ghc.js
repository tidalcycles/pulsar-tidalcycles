...
export default class Ghc {
  ...
  init() {
    switch (atom.config.get(interpreterProperty)) {
      case 'stack':
        this.interactivePath = 'stack exec --package tidal ghci'
        this.pkgPath = 'stack exec --package tidal ghc-pkg'
        break;
      case 'nix':
        this.interactivePath = 'nix-shell -p "haskellPackages.ghcWithPackages (pkgs: [pkgs.tidal])" --run ghci'
        this.pkgPath = 'nix-shell -p "haskellPackages.ghcWithPackages (pkgs: [pkgs.tidal])" --run "ghc-pkg"'
        break;
      default:
  ...
  browseTidal(callback) {
    let browseTidalCommand = atom.config.get(interpreterProperty) === 'default'
      ? `echo ":browse Sound.Tidal.Context" | "${this.interactivePath}"`
      : `echo ":browse Sound.Tidal.Context" | ${this.interactivePath}`

    child_process.exec(browseTidalCommand, (error, stdout) => {
      if (error) {
