...
export default class EventHighlighter {
  ...
  init() {
    ...
    // Local OSC server for editor‑events
    this.server = new OscServer(this.consoleView, atom.config.get('tidalcycles.eventHighlighting.oscPort'));
    this.server.start();
    this.server.sock.on("message", (msg) => this.#handleEventMessage(msg));
  ...
  addMetadata(line, lineNumber) {
    ...
    return line.replace(/"([^"]*)"/g, (match, content, offset) => {
      ...
      return `(deltaContext ${offset} ${lineNumber} "${content}")`;
    });
  }
  ...
  #handleEventMessage(msg) {
    const { args } = osc.fromBuffer(msg).elements[0];
    if (!args || args.length < 7) return;

    const [id, , , colStart, rowStart, colEnd, rowEnd] = args.map((a) => a.value);
    const event = { id, colStart, rowStart: rowStart - 1, colEnd, rowEnd: rowEnd - 1 };
    this.#queueEvent(event);
  }

  /** Store events until the next animation frame */
  #queueEvent(event) {
    const rowMap = ensureNestedMap(this.filteredMessages, event.rowStart);
    const recvMap = ensureNestedMap(this.receivedThisFrame, event.rowStart);
  ...
  animate(now) {
    ...
    if (elapsed >= fpsInterval && isFeatureEnabled) {
      ...
      this.addedThisFrame.forEach((evt) => this.#addHighlight(evt));
  ...
  #addHighlight({ id, colStart, rowStart }) {
    if (!this.editor) return;

    const rowMarkers = this.markers.get(rowStart);
    const baseMarker = rowMarkers?.get(colStart);
    if (!baseMarker?.isValid()) return;

    const highlightRow = ensureNestedMap(this.highlights, rowStart);
  ...
  #removeHighlight({ colStart, rowStart }) {
    const highlightRow = this.highlights.get(rowStart);
