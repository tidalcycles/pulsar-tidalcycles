...
export default class TidalListenerRepl {
  ...
  highlight(arguments) {
    // TODO: double quotes should be not contained in id
    let id = arguments[0].value.replace('\"', '').replace('\"', '')
    let duration = arguments[1].value
    let cyclePosition = arguments[2].value
    let startColumn = arguments[3].value
    let startRow = arguments[4].value - 1
    let stopColumn = arguments[5].value
    let stopRow = arguments[6].value - 1
    let detail = arguments.map(it => it.value).join(',')

    if (!this.highlights[id]) {
      this.highlights[id] = { cyclePosition: 0, ranges: [], details: [] }
    }

    if (this.highlights[id].cyclePosition !== cyclePosition) {
      this.highlights[id].cyclePosition = cyclePosition
      this.highlights[id].details = []
      this.highlights[id].ranges = []
    }

    if (!this.highlights[id].details.includes(detail)) {
      this.highlights[id].details.push(detail)
      this.highlights[id].ranges.push([[startRow, startColumn], [stopRow, stopColumn]])

      this.editors.currentHighlight(this.highlights)
    }
  }
